...
static struct platform_driver rk817_battery_driver = {
	.probe = rk817_battery_probe,					// 探测函数，当匹配到设备时调用（不只是挂载）
	.shutdown = rk817_battery_shutdown,				// 系统关机时调用的函数（不仅仅是驱动关闭）
	// 驱动配置信息
	.driver = {
		.name = "rk817-battery",								// 驱动名称
		.pm = &rk817_bat_pm_ops,								// 电源管理操作
		.of_match_table = of_match_ptr(rk817_bat_of_match),		// 设备树匹配表
	},
};
