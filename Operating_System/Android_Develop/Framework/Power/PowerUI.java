...
public class PowerUI extends SystemUI implements CommandQueue.Callbacks {
    ...
    // xxx:手探
    private final static String SHOUTAN_PATH="/sys/kernel/vcc_uart/uart5";
    // xxx: lora
    private final static String LORA_PATH="/sys/kernel/vcc_uart/uart4";
    /**
     * xxx: 向设备节点写入内容
     */
    private  void wirteGpio(String command, String path){
        Slog.d(TAG,"xxx: wirteGpio command:"+command+",path:"+path);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Slog.d(TAG, "xxx onReceive action:"+action );
            if (PowerManager.ACTION_POWER_SAVE_MODE_CHANGED.equals(action)) {
                ...
                Slog.d(TAG, "xxx  mBatteryLevel:"+mBatteryLevel );
                //if(mBatteryLevel <= 20){
                ...
                Slog.d(TAG, "xxx  ACTION_SCREEN_OFF");
                //wirteGpio("0",SHOUTAN_PATH);
                //wirteGpio("0",LORA_PATH);
        ...
        private  void executeSequentially() {
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    wirteGpio("1",LORA_PATH);
                    Slog.d(TAG, "xxx  sleep start");
                    try {
                        Thread.sleep(1000); // 延迟1秒
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    Slog.d(TAG, "xxx  sleep end");
                    wirteGpio("1",SHOUTAN_PATH);
                }
            });
    ...
    public interface WarningsUI {
        ...
        // 在 mBucket 从 ≥0 跌到 <0 时调用
        void showLowBatteryWarning(boolean playSound);

        void dismissInvalidChargerWarning();

        void showInvalidChargerWarning();
        // 电量维持在低区间时会反复调用，用于更新对话框内容
