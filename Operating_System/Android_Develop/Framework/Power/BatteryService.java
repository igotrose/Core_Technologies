...
public final class BatteryService extends SystemService {
    ...
    // 构造函数，初始化各种状态、读取资源、准备LED、注册UEvent监听
    ...
    // 注册 ContentObserver，监听低电量阈值的设置变化
    @Override
    ...
    private boolean shouldShutdownLocked() {
        ...
        Slog.d(TAG,"xxx361xx.............isNotCharging="+isNotCharging+",batteryVoltage="+mHealthInfo.batteryVoltage
                 +"batteryLevel="+mHealthInfo.batteryLevel);
        if (!isNotCharging) {
            return false;
        }

        if (mHealthInfo.batteryVoltage > 3400) {
            Slog.d(TAG,"xxx......>3.4........>> shutdown_vol");
			return false;
		}
        ...
        if (mHealthInfo2p1.batteryCapacityLevel != BatteryCapacityLevel.UNSUPPORTED) {
            if (mHealthInfo2p1.batteryCapacityLevel == BatteryCapacityLevel.CRITICAL) {
                Slog.w(TAG, "xxx  batteryCapacityLevel is CRITICAL need Shutdown");
                return true;
        ...
        if (mHealthInfo.batteryLevel > 0 ) {
            Slog.d(TAG,"xxx........baterry level >0......>> shutdown_vol");
            return false;
        }
    ...
    // 电压低于 3450mV 或 6% 并且没有插电时，发起关机
    private void shutdownIfNoPowerLocked() {
        // shut down gracefully if our battery is critically low and we are not powered.
        // wait until the system has booted before attempting to display the shutdown dialog.
        //if (shouldShutdownLocked()) {
        //Slog.d(TAG,"xxx...batse...shutdown vol:"+mHealthInfo.batteryVoltage +",level:"+mHealthInfo.batteryLevel);
        boolean shutdownFlag =false;
        if(mHealthInfo.batteryVoltage < 3450){
            shutdownFlag =true;
        }
        if(mHealthInfo.batteryLevel<6 && !isPoweredLocked(BatteryManager.BATTERY_PLUGGED_ANY)){
            shutdownFlag =true;
        }
        if (shutdownFlag){
            Slog.d(TAG,"xxx  shouldShutdownLocked true shutdown");
